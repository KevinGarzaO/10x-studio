...
- [ ] T006 [P] Unit tests del schema compartido en `backend/tests/unit/skillExam.schema.test.ts`: un caso que falla y uno que pasa por cada regla (`selectedOptionIndex` negativo y `> 5`, `position` negativa, `skillName` fuera de la lista de skills válidos y dentro de ella)
