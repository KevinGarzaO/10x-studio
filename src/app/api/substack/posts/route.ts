...
export async function GET() {
  try {
    ...
    // Check if we have data and it's fresh enough (e.g. 30 mins for posts)
    // We'll use the most recent 'synced_at'
    const lastSynced = postsInDb.length > 0 
      ? new Date(postsInDb[0].synced_at).getTime() 
      : 0
    const now = Date.now()
    const thirtyMins = 30 * 60 * 1000

    if (postsInDb.length > 0 && (now - lastSynced < thirtyMins)) {
    ...
    const url = `https://${pubSlug}.substack.com/api/v1/post_management/published?offset=0&limit=25&order_by=post_date&order_direction=desc`
