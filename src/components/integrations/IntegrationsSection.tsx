...
export function IntegrationsSection() {
  ...
  async function saveWp() {
    setSaving('wp')
    const res = await fetch('/api/integrations', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ wpUrl: wpUrl.trim(), wpUsername: wpUser.trim(), wpAppPassword: wpPass, wpDefaultStatus: wpStatus }) })
    setSaving(null)
    if (res.ok) { showMsg('ok', '✅ WordPress guardado'); loadCfg() }
    else showMsg('err', '❌ Error guardando WordPress')
  }
  ...
  async function saveLi() {
    ...
    setSaving(null)
    if (res.ok) { showMsg('ok', '✅ LinkedIn guardado'); loadCfg() }
    else showMsg('err', '❌ Error guardando LinkedIn')
  }
