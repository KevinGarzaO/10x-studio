...
class SubstackService {
    ...
    static async syncProfile(userId, substackUserId, substackSlug) {
        ...
        const res = await (0, node_fetch_1.default)(url, { headers: this.getHeaders(cookie) });
        if (!res.ok)
            throw new Error(`Substack API error: ${res.status} para ${url}`);
        ...
        const { error: userErr, data: updatedUser } = await supabase_service_1.supabase
            .from('users')
            .upsert(userData, { onConflict: 'substack_user_id' })
            .select('id')
            .single();
        if (userErr)
            console.error('[Substack] Error upsert users:', userErr);
        ...
        if (profile.primaryPublication) {
            ...
            const { error: pubErr } = await supabase_service_1.supabase.from('publications').upsert(pubData, { onConflict: 'id' });
            if (pubErr)
                console.error('[Substack] Error upsert publications:', pubErr);
        }
        return { profile };
    }
    static async syncPosts(userId, subdomain) {
        const cookie = await this.getCookieHeader(userId);
        if (!cookie)
            throw new Error('No cookies found');
        ...
        const res = await (0, node_fetch_1.default)(url, { headers: this.getHeaders(cookie, `https://${subdomain}.substack.com`) });
        if (!res.ok)
            throw new Error(`Substack API error: ${res.status}`);
        const posts = await res.json();
        const postsArray = Array.isArray(posts) ? posts : (posts.posts || []);
        const postsToUpsert = postsArray.map((p) => ({
            user_id: userId,
            post_id: String(p.id),
            title: p.title,
            subtitle: p.subtitle,
        ...
        if (postsToUpsert.length > 0) {
            await supabase_service_1.supabase.from('posts').upsert(postsToUpsert, { onConflict: 'user_id,post_id' });
        }
        return postsToUpsert;
    }
    static async syncStats(userId, subdomain) {
        const cookie = await this.getCookieHeader(userId);
        if (!cookie)
            throw new Error('No cookies found');
        ...
        await supabase_service_1.supabase.from('stats').upsert(newStat, { onConflict: 'user_id,date' });
        // Also update user summary
        await supabase_service_1.supabase.from('users').update({ subscriber_count: subscriberCount }).eq('id', userId);
        return newStat;
    }
    static async syncSubscribers(userId, subdomain) {
        const cookie = await this.getCookieHeader(userId);
        if (!cookie)
            throw new Error('No cookies found');
        ...
        if (allSubscribers.length > 0) {
            ...
            const subsToUpsert = allSubscribers.map((s) => ({
                user_id: userId,
            ...
            const uniqueSubs = subsToUpsert.filter((sub, index, self) => index === self.findIndex(s => s.email === sub.email));
            const { error } = await supabase_service_1.supabase.from('subscribers').upsert(uniqueSubs, { onConflict: 'user_id,email' });
            if (error) {
    ...
    static async createDraft(userId, params) {
        ...
        const headers = this.getHeaders(cookie, `https://${subdomain}.substack.com`);
        const res = await (0, node_fetch_1.default)(`https://${subdomain}.substack.com/api/v1/drafts`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                draft_title: '',
    ...
    static async updateDraft(userId, draftId, params) {
        const cookie = await this.getCookieHeader(userId);
        const { data: user } = await supabase_service_1.supabase.from('users').select('subdomain, substack_user_id').eq('id', userId).single();
        if (!user)
            throw new Error('User not found');
    ...
    static async scheduleDraft(userId, draftId, scheduleAt) {
        const cookie = await this.getCookieHeader(userId);
        const { data: user } = await supabase_service_1.supabase.from('users').select('subdomain').eq('id', userId).single();
        if (!user)
            throw new Error('User not found');
        const subdomain = user.subdomain;
        const headers = this.getHeaders(cookie, `https://${subdomain}.substack.com`);
        const res = await (0, node_fetch_1.default)(`https://${subdomain}.substack.com/api/v1/drafts/${draftId}/scheduled_release`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
    ...
    static async addSubscriber(userId, email) {
        const cookie = await this.getCookieHeader(userId);
        const { data: user } = await supabase_service_1.supabase.from('users').select('publication_id, subdomain').eq('id', userId).single();
        if (!user)
            throw new Error('User not found');
        const pubId = user.publication_id;
        const subdomain = user.subdomain;
        const headers = this.getHeaders(cookie, `https://${subdomain}.substack.com`);
        const res = await (0, node_fetch_1.default)(`https://substack.com/api/v1/publication/${pubId}/subscribers/import`, {
            method: 'POST',
            headers,
