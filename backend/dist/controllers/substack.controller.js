...
const getProfile = async (req, res) => {
    try {
        ...
        const { data: cookiesRow } = await supabase_service_1.supabase
            .from('cookies')
            .select('expires_at')
            .eq('user_id', user.id)
            .single();
        ...
        const { data: publications } = await supabase_service_1.supabase
            .from('publications')
            .select('*')
            .eq('user_id', user.id);
        const pubs = publications || [];
        console.log('[getProfile] Publications:', pubs.map((p) => ({ name: p.name, subdomain: p.subdomain })));
        const primaryPub = pubs.find((p) => p.is_primary) || pubs[0];
        // 4. Build response
        const responseData = {
            ...user,
...
const debugDB = async (_req, res) => {
    try {
        const { data: users } = await supabase_service_1.supabase.from('users').select('*').limit(3);
        const { data: pubs } = await supabase_service_1.supabase.from('publications').select('*').limit(3);
        const { data: cookies } = await supabase_service_1.supabase.from('cookies').select('*').limit(3);
        const { data: subs, count } = await supabase_service_1.supabase.from('subscribers').select('*', { count: 'exact' }).limit(3);
        // Probe subscribers schema by trying a test insert (will fail but reveals columns)
        const { error: subSchemaErr } = await supabase_service_1.supabase.from('subscribers').insert({
            user_id: 'test-probe',
        ...
        await supabase_service_1.supabase.from('subscribers').delete().eq('email', 'probe@test.com');
        // Probe publications schema
        const { error: pubSchemaErr } = await supabase_service_1.supabase.from('publications').insert({
            publication_id: 0,
            name: 'probe',
            subdomain: 'probe',
            author_id: 0
        });
        await supabase_service_1.supabase.from('publications').delete().eq('subdomain', 'probe');
        res.json({
            users: {
...
const getSubscribers = async (req, res) => {
    try {
        ...
        const { data, error, count } = await supabase_service_1.supabase
            .from('subscribers')
            .select('*', { count: 'exact' })
            .eq('user_id', user.id)
        ...
        const { count: topLeadsCount } = await supabase_service_1.supabase
            .from('subscribers')
            .select('id', { count: 'exact', head: true })
            .eq('user_id', user.id)
            .gte('stars', 4);
        // AVG ESTRELLAS: promedio de estrellas
        const { data: starsData } = await supabase_service_1.supabase
            .from('subscribers')
            .select('stars')
            .eq('user_id', user.id)
            .not('stars', 'is', null);
...
const getStats = async (req, res) => {
    try {
        const { data: user } = await supabase_service_1.supabase.from('users').select('id, subscriber_count, follower_count').single();
        if (!user)
            return res.status(404).json({ error: 'Usuario no encontrado' });
        const { data } = await supabase_service_1.supabase
            .from('stats')
            .select('*')
            .eq('user_id', user.id)
...
const createDraft = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ error: 'Usuario no encontrado' });
        const result = await substack_service_1.SubstackService.createDraft(user.id, req.body);
        res.json(result);
    }
    catch {
...
const updateDraft = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ error: 'Usuario no encontrado' });
        const { id } = req.params;
        const result = await substack_service_1.SubstackService.updateDraft(user.id, id, req.body);
        res.json(result);
    }
    catch {
...
const scheduleDraft = async (req, res) => {
    try {
        ...
        const result = await substack_service_1.SubstackService.scheduleDraft(user.id, id, scheduleAt);
        res.json(result);
    }
    catch {
...
const addSubscriber = async (req, res) => {
    try {
        ...
        const result = await substack_service_1.SubstackService.addSubscriber(user.id, email);
        res.json(result);
    }
    catch {
...
const getCookies = async (req, res) => {
    try {
        ...
        const { data, error } = await supabase_service_1.supabase.from('cookies').select('*').eq('user_id', userId).maybeSingle();
        if (error)
            throw error;
        res.json(data);
    }
...
const upsertCookies = async (req, res) => {
    try {
        ...
        let { data: users } = await supabase_service_1.supabase.from('users').select('id, substack_slug').eq('substack_user_id', substackUserId).limit(1);
        let user = users?.[0];
        if (!user) {
            const { data: newUser, error: insertError } = await supabase_service_1.supabase.from('users').insert({
        ...
        await supabase_service_1.supabase.from('cookies').upsert(cookieData, { onConflict: 'user_id' });
        ...
        try {
            await (0, cron_service_1.syncSubstackData)(user.id);
        ...
        const { data: finalUser } = await supabase_service_1.supabase.from('users').select('*').eq('id', user.id).single();
        // Get cookies separately
        const { data: cookiesRow } = await supabase_service_1.supabase.from('cookies').select('expires_at').eq('user_id', user.id).single();
        const finalExpiresAt = cookiesRow?.expires_at || expiresAt.toISOString();
        // Get publications by user_id
        const { data: pubs } = await supabase_service_1.supabase.from('publications').select('*').eq('user_id', user.id);
        const publications = pubs || [];
        const primaryPub = publications.find((p) => p.is_primary) || publications[0];
...
exports.upsertCookies = upsertCookies;
const deleteCookies = async (req, res) => {
    try {
        const { data: user } = await supabase_service_1.supabase.from('users').select('id').single();
        if (!user)
            return res.status(404).json({ error: 'Usuario no encontrado' });
        const { error } = await supabase_service_1.supabase.from('cookies').delete().eq('user_id', user.id);
        if (error)
            throw error;
        res.json({ ok: true });
    }
